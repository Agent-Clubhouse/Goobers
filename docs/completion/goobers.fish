...
complete -c goobers -n '__fish_seen_subcommand_from engine-start' -l task-queue -r -d 'Task queue to dispatch onto'
...
complete -c goobers -n '__fish_seen_subcommand_from engine-start' -l dedupe-key -r -d 'Dedupe key folded into the run id'
