...
| `failure` | If the task declares `continueOnError: true`, journal the failed status plus a distinct tolerated-failure note, discard its outputs, and advance to `Next` as if it succeeded; a following gate still observes the honest failed status. Otherwise, **non-retryable escalate disposition applies first (#415):** if `error.retryable == false` **and** `error.code` is a recognized escalate code (`ISSUE_OVER_SCOPE` / `NEEDS_DECOMPOSITION`), bypass the `Next` gate's evaluator and route through its optional `escalate` control branch; without one, terminate directly at `@escalate`. For other failures, advance only when `Next` is a gate (the reviewer-gate pattern); a non-gate, terminal, or empty `Next` ends the run `PhaseFailed`. |
