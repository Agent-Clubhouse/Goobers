...
| `no-work` | finish the run `completed` without evaluating the task's declared next state — **unless the claim is unsubstantiated (#2736)**. A stage that consumes upstream evidence *by declaration* — a task naming its producers with `contextFrom`, or a parallel's fan-in join — is checked against the journal before the claim is accepted: if none of those upstream stages journaled any artifact or output, the evidence never arrived, the claim has no basis, and the run ends `PhaseFailed` with `NO_WORK_UNSUBSTANTIATED` naming the barren upstream (a `runner.annotation` records it too). One producing upstream stage is enough for the claim to stand. Stages that declare no such upstream are unaffected: a predecessor can deliver through the shared workspace and return an empty envelope. |
