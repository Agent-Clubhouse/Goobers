...
| # | Decision | Why |
| --- | --- | --- |
| D1 | Typed `run.kind`, landing TBH-1 ([stage-contract.md](../stage-contract.md) §"runner-owned deterministic run kinds"), with `external-call` as its first value | `inputs.kind` is runtime-overridable: `internal/runner/run.go` (`dispatchTask`) overlays every `inputsFrom` key onto `env.Inputs` immediately before dispatch and `internal/executor/dispatch.go` (`TaskExecutor.Run`) selects the executor from the overlaid map, while admission reads only the static literal (`internal/workflow/v_next/compile.go` (the two `t.Inputs["kind"]` literals)). A stage can read as `shell` to a reviewer and dispatch a call-out |
...
| D3 | **Every artefact is Task-scoped** | `FeaturesForGaggle`, `FeaturesForGoober` and `CheckFeatureSupport` call `vcurrent.*` unconditionally (`internal/workflow/features.go` (`FeaturesForGaggle`, `FeaturesForGoober`, `CheckFeatureSupport`)), so a v_next-only feature at goober or gaggle scope is invisible to introspection. Task scope avoids the asymmetry rather than fixing it; routing those functions through `interpreterForDefinition` is a prerequisite for any later goober-scoped surface (filed, §10) |
| D4 | DSL **2.0 preview feature**, not a 2.1 bump | A minor costs a copied interpreter (each interpreter is ~9k lines), a hand-edited arm, a migration edge, and a 3-release support commitment validated at package init (`internal/supportmatrix/supportpolicy.go` (`MinimumSupportWindowMinorReleases`)) |
...
| D9 | The reconcile decision **splits across the seam**: `attemptFailureClass` returns `reconcileNext`, the dispatch closure acts on it | `attemptFailureClass` (`internal/engine/retry.go`, `attemptFailureClass`) runs inside the workflow function with only the task and error available; `effect` lives in `instance.yaml`, which the in-workflow re-compile (`internal/engine/engine.go`, the in-workflow `wf.Compile`) must not read |
| D10 | An explicit **ambiguous outcome** (`callout_outcome_unknown`) | `ResultStatus` is success/failure/blocked/no-work (`api/v1alpha1/envelope.go`, `ResultStatus`). Today an unknown must masquerade as failure, which a gate acts on |
| D11 | The kind owns its error: `Retryable` constant `false`, `Code` from a closed `callout_*` vocabulary | `AutomatedInputs` fills the error triple from `subject.Error` (`internal/gate/automated.go`, `AutomatedInputs`), and `failure-class` forks on it. The landed #2925 fix closes the *Outputs* door; it does not touch this one |
| D12 | **Sync + submit-then-poll**, modelled on `ci-poll`. No async callback | No landing zone: `internal/instance/config_validation.go` refuses a non-loopback bind with "there is no insecure override"; [k8s-infra-shape.md](k8s-infra-shape.md) §5 "No inbound to stage pods. Ever."; `ResultBlocked` is terminal, not resumable; the engine refuses human gates outright |
...
| D14 | **No new journal event types.** `stage.*` + `artifact.recorded` + `ref.touched` | `IsConformanceNormative` ends `default: return true` (`internal/journal/event.go`, `IsConformanceNormative`), so a new type must be emitted byte-identically by both runners — impossible against a live endpoint. `projectableEventTypes` (`internal/engine/projection.go`, `projectableEventTypes`) is a closed ten-entry whitelist validated over *all* ops before `journal.Create`, so an unknown type yields **no journal at all**. `artifact.recorded` is exempt: the projection's artifact op writes it through the journal writer rather than as an op |
...
| D16 | Callee identity is structured `(provider, kind, id)`, never a URL | `ExternalRef.URL` is dropped from the conformance projection (`internal/journal/conformance.go`) |
...
| D18 | A **produced-integrity floor**, as a runner change | `internal/runner/run.go` (`runTask` overwrites `result.Integrity` with `producedIntegrity`) unconditionally overwrites the executor's grade, and `internal/runner/inputsfrom.go` (`producedIntegrity`) returns `IntegrityTrusted` for a stage with no graded input — the common call-out shape. Without the floor the far side's scalars reach downstream stages labelled trusted |
...
| D20 | Target-scoped grants `callout:invoke@<target>` | Follows `credentials.RepoScopedCapability` (`internal/credentials/scoping.go` (`RepoScopedCapability`)), already opaque to the injector. A flat capability would make operator curation all-or-nothing |
...
| D23 | Tracing: **correlate now, stitch later** | The run ID *is* the 32-hex trace ID (`internal/telemetry/id.go` (`runIDGenerator`)), so correlation is free. `traceparent` from a `SpanKindClient` span needs one small change (`telemetry.Span` has no `SpanContext` accessor). Stitching stays deferred because no OTLP receiver exists and foreign parents are rejected — **not** because tier 3 lacks spans, which stopped being true when #2865 landed |
| D24 | A far-side `Retry-After` is **clamped** by the kind | Nothing clamps it today: `internal/runner/run.go` (`infrastructureRetryDelay`) returns it verbatim and tier 3 turns it into a durable `workflow.Sleep` (`internal/engine/retry.go`, `dispatchWithRetry`). A hostile `503` + far-future `Retry-After` parks the stage past its own timeout |
...
**`no-phone-home` is not the egress control.** `test/nophonehome/main.go` (`isCommandFile`) gates `.yml`/`.yaml`
...
| --- | --- | --- |
| Gate verdict forgery via `Outputs` | `AutomatedInputs` used to copy subject `Outputs` over the runner-set `status` | **Closed on `main`.** #2925 landed: outputs are copied first, reserved keys stamped after, and a collision returns an error (`internal/gate/automated.go`, `AutomatedInputs`) |
...
2. **The `TimeoutError.TimeoutType()` split** — a prerequisite rather than a follow-up, because a mutating
   call-out plus blind infra retry is a duplicate-submit generator (`internal/engine/retry.go`, `attemptFailureClass`).
