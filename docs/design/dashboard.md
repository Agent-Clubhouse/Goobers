...
The portal closes its connection while the tab is hidden, preserves accepted
invalidations still awaiting refresh, and resumes with its last cursor when
visible again. An accepted resume refreshes only subscribers matched by queued
or replayed invalidations; it does not trigger a blanket refetch. A full refresh
occurs only for the initial snapshot, after a missing or stale cursor, or while
polling as the recovery fallback.
...
projection, consistent with the shared-read-service architecture. The projection
lifecycle, query model, ordering, freshness contract, and follow-up boundaries
are specified in
[`portal-read-architecture.md`](portal-read-architecture.md), which supersedes
the earlier [`unified-index-backed-run-reads.md`](unified-index-backed-run-reads.md).
