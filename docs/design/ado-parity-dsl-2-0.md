...
| ADO-N21 | `goobers:ready` / label transitions (#5554) | Implement `ListWorkItemLabelTransitionsForItem` (`ado_workitems.go:799`) from `GET workitems/{id}/updates`: `System.Tags` old/new diffs and `revisedDate`, paged with `$top`/`$skip`, and a fail-closed error past the 10,000-revision cap. | features §3.9 |
