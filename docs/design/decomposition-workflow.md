...
| **DEC-4 — Prepared-batch publisher and eligibility barrier** | DEC-2, DEC-3 | A deterministic publisher creates quarantined shells, resumes every crash boundary, verifies the complete batch, and commits through one parent marker. `backlog-query` cannot claim any marked child before that commit. Tests inject a failure after every provider mutation. |
