...
| D10 | **Admission is three checkpoints**: apply-time constraint solve (error when a `runners:` inventory is declared, warning otherwise — fixes the #3497 severity trap for the declared case); bounded fail-fast at dispatch; boot **never** kills the daemon (implements #2860's ruling). Inventory updates are accept-and-pin | D4 | Apply-time truth rots (spot eviction, scaled-to-zero pools), so capability-unsatisfiable is an apply error while capacity-exhausted is a bounded, named runtime failure. At daemon startup, `CheckCapabilityRequirements` now produces an operator warning and startup continues; affected runs are refused at schedule time. Accept-and-pin also answers #3449: no drain machinery is required for config updates |
