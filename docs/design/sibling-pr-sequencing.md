...
| 4c | dependency-recheck re-sweep | An issue whose blockers are native GitHub issue dependencies is selected by `appendBlockedResweepCandidates` into a bounded daily `dependency-recheck` curation run once those dependencies resolve, so a curator decides whether the work deserves another attempt. That selection **requires the park label**, which is why row 4b must not clear it: reconciliation runs first in the same `backlog-query --claim --curation` invocation, so clearing there would drop the item out of this lane and into ordinary eligibility. | `backlogquery.go` |
