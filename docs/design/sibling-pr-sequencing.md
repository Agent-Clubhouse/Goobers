...
> a review artifact held outside this repository and **not reproducible from this
> repository**; every claim this design depends on is restated below,
...
| 4b | issue-side unpark | An **issue** parked `blocked-on-sibling` is unparked during backlog reconciliation by `staleBlockedOnSiblingMarker` (#3355/#4545). It clears the marker only on positive proof: the learned block ledger or the comment payload names blockers and every one has resolved — by merging **or by being closed by hand**, and whether the blocker is a PR or an issue. Clearing never re-adds `goobers:ready`. Blockers recorded only as **native GitHub issue dependencies** are deliberately left alone here; they belong to row 4c. | `blockedonsibling.go`; `backlogreconcile.go` |
| 4c | dependency-recheck re-sweep | An issue whose blockers are native GitHub issue dependencies is selected by `appendBlockedResweepCandidates` into the separately scheduled `curate-resweep` workflow once those dependencies resolve, so a curator decides whether the work deserves another attempt. Selection requires the park label, so row 4b must preserve it: clearing it during ordinary reconciliation would move the item into forward eligibility instead of this bounded recheck lane. The query uses `backlog-query --claim --resweep`; its schedule/readiness owns cadence and provider errors fail its own journaled stage (#2399). | `backlogquery.go`; `curate-resweep.yaml` |
