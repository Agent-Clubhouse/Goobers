...
| DI-14 | **Derived capability tags (`run:shell`, `harness:<name>`) are satisfied implicitly by base-image identity, never by an authored `provides.capabilities` claim** (#3513, resolving the open point `internal/runnercap` names). A pod dispatched from `goobers-base` (by base-layer digest match, never tag/name string matching) implicitly satisfies `run:shell`; a pod from `goobers-harness-<name>` implicitly satisfies `harness:<name>`. The author-token grammar stays closed to the derived namespace exactly as it is today — no config surface, no new field. | Generalizes the existing `self`-implicit rule (record D3: "the daemon host runs every configured harness and every shell stage… preflight-verified at startup") to the other case where the platform, not a self-report, is the source of the fact: DI-1's product-owned images are ones the dispatcher already knows the contents of. Keeps DI-11's trusted-claims problem out of the derived namespace — an authored claim here would be an unverifiable promise about arbitrary contents, exactly what DI-11 already flags as a residual risk for ordinary capabilities; a digest-identified product image is not that kind of claim. **Accepted residual, not fixed by this decision**: image layering can remove a capability after the base (e.g. a later layer deleting `/bin/sh`) — DI-14 identifies the image, it does not verify runtime contents survive. A stage that places under this rule and fails at exec is diagnosing that residual, not a contract violation. |
...
- **`goobers-harness-copilot` / `goobers-harness-claude`** add exactly one agent-harness
  ...
  what discharges the derived `harness:<name>` requirement.
