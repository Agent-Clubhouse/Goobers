...
| DI-3 | `goobers-base` is near-distroless: the `goobers` binary, CA certificates, git, tz data — nothing else. Harness variants layer exactly one agent-harness runtime on top of base. | The shipped all-in-one image (packaging/docker/Dockerfile: Node 22 Debian base carrying goobers + operator + gh + Copilot CLI) is **superseded as the published shape** — it remains the ancestor and the single-image convenience for mode-2 reference deployments, but the published set separates control-plane from harness so deterministic stages never pay a Node runtime pull. tz data is on the required list because its absence in Windows base images broke the first OS-spanning run (#2863). |
...
| DI-6 | **Image tag = goobers binary version** inside the image, and v1 requires tag equality with the dispatching daemon: the dispatcher launches stage pods from the image whose tag equals its own binary version. | This is the version-skew contract (record D8). Skew between daemon and worker binaries is the recorded #1061 failure-mode class; equality-by-construction is the cheapest correct answer. A tolerated skew window is deliberately not designed in v1 (open point O-1). |
...
1. `docker inspect` of every published image shows a `goobers` (or `goobers.exe`) on
   `PATH` whose `goobers --version` output equals the image tag; a CI check runs the
   comparison per release and fails on mismatch (DI-6, DI-7).
