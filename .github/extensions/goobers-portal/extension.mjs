...
// The runtime-provided @github/copilot-sdk isn't resolvable when the
// extension is imported outside the Copilot host (for example from Node
// --test suites that only need to inspect canvas declarations, action
// handlers, and HTTP dispatch). GOOBERS_PORTAL_TEST swaps in tiny local
// stand-ins that preserve just enough shape (the declaration is passed
// through unchanged; joinSession returns the canvases plus a no-op event
// handle; CanvasError still carries a code) for behavioural tests to
// exercise real handlers without a live SDK. Everywhere else we still
// dynamically import the real SDK so runtime behaviour is unchanged.
const { createCanvas, joinSession, CanvasError } = process.env.GOOBERS_PORTAL_TEST
    ? {
        createCanvas: (declaration) => ({ declaration }),
        joinSession: async ({ canvases }) => ({ canvases, on: () => {}, sessionId: "test-session" }),
    ...
    : await import("@github/copilot-sdk/extension");
