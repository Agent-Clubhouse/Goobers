...
// A navigation-retention window, not a staleness bound (#1928).
//
// Scoped live invalidations keep retained data current. The longer TTL lets a
// user move between pages without replacing known content with loading chrome;
// mounted hooks still refresh and publish stale state while that refresh runs.
export const DATA_CACHE_TTL_MS = 5 * 60_000;
