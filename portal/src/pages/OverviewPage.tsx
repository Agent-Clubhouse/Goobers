...
import { instanceWarnings, runs, workflowForRun, workflows } from "../prototypeData";
    ...
    <>
      ...
      <section className="content-section">
        ...
        <DataList
          ...
          headerClassName="run-grid"
          headings={["Run", "Workflow", "Current stage", "Elapsed", ""]}
        >
          {activeRuns.map((run) => (
      ...
      <section className="content-section">
        ...
        <DataList
          ...
          headerClassName="outcome-grid"
          headings={["Run", "Outcome", "Workflow", "Duration", ""]}
        >
          {recentRuns.map((run) => (
            <DataRow key={run.id} label={`Open run ${run.title}`} onClick={() => navigate({ page: "run", id: run.id })}>
              <span className="row-primary">
