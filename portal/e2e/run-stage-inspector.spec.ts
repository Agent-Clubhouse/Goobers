...
async function openInspector(page: Page): Promise<{
  attemptRequests: () => number;
  ...
  return {
    attemptRequests: () => attemptRequests,
    invalidate: async () => {
      await expect.poll(async () => {
        const response = await page.request.post("/api/v1/test/invalidate");
...
test("expanded artifact remains visible across three background refreshes", async ({ page }) => {
  ...
  const { attemptRequests, invalidate } = await openInspector(page);
  ...
  for (let refresh = 1; refresh <= 3; refresh += 1) {
    await invalidate();
    await expect.poll(attemptRequests).toBeGreaterThanOrEqual(baseline + refresh);
  }
  await expect(page.getByText(content)).toBeVisible();
});
