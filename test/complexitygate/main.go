...
func run(args []string, stdout, stderr io.Writer) int {
	...
	committed, err := readCommittedBaseline(*root, resolved)
	if err != nil {
		_, _ = fmt.Fprintf(stderr, "complexitygate: read committed baseline: %v\n", err)
		return 1
	}
	if committed != nil {
		if err := validateBaselineUpdate(committed, base); err != nil {
			_, _ = fmt.Fprintf(stderr, "complexitygate: baseline differs from HEAD without an exact-target justification: %v\n", err)
			return 1
		}
	}
...
func evaluateBodyLengths(functions []function, base baseline, limit int) (problems, notes []string) {
	...
	for entryKey, recorded := range base.BodyLengths {
		...
		path, symbol, _ := strings.Cut(entryKey, "\t")
		problems = append(problems, fmt.Sprintf(
			"complexitygate: body-length baseline entry %s %s at %d does not match the sealed current ceiling %d",
			path, symbol, recorded, ceiling,
		))
	}
	seen := make(map[string]bool)
	for _, current := range functions {
...
func writeBaseline(path string, functions []function, limits thresholds, previous, candidate *baseline) error {
	...
	return writeBodyLengthSeed(filepath.Join(filepath.Dir(path), "body_length_seed.go"), next.BodyLengths)
}

func writeBodyLengthSeed(path string, ceilings map[string]int) error {
	keys := make([]string, 0, len(ceilings))
	for entryKey := range ceilings {
		keys = append(keys, entryKey)
	}
	sort.Strings(keys)
	var builder strings.Builder
	builder.WriteString("package main\n\n")
	builder.WriteString("// Code generated by `make complexity-update`; DO NOT EDIT.\n")
	builder.WriteString("// bodyLengthSeedCeilings is #4847's sealed, currently-approved inventory.\n")
	builder.WriteString("// Every active baseline row must exactly match its ceiling here, so a baseline-only\n")
	builder.WriteString("// rebound remains visible as a reviewed control-code change.\n")
	builder.WriteString("var bodyLengthSeedCeilings = map[string]int{\n")
	for _, entryKey := range keys {
		fmt.Fprintf(&builder, "\t%q: %d,\n", entryKey, ceilings[entryKey])
	}
	builder.WriteString("}\n")
	formatted, err := format.Source([]byte(builder.String()))
	if err != nil {
		return fmt.Errorf("format body-length seed: %w", err)
	}
