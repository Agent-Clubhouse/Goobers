...
func writeBaseline(path string, functions []function, limits thresholds) error {
	...
	builder.WriteString("#\n")
	builder.WriteString(fmt.Sprintf("# Entries are every function at or above the hard cap of %d, keyed by\n", limits.hardCap))
	...
	builder.WriteString(fmt.Sprintf("# %s is how many functions may sit at or above %d.\n", budgetDirective, limits.ratchet))
