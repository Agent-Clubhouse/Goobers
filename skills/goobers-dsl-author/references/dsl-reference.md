...
failure with a recognized host-contention or dependency-download transport
signature, and `fail` for every other status. Use only outcomes and parameters
accepted by the target release. Agentic gates must cover `pass`, `fail`, and
`needs-changes`. Human gates may be present in a schema before a runner supports
them, so always confirm them with `goobers validate`.
