...
failure with a recognized host-contention or transient-transport signature, and
`fail` for every other status. Use only outcomes and parameters accepted by the
target release. Agentic gates must cover `pass`, `fail`, and `needs-changes`. Human
