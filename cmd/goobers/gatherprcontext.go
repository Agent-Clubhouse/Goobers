...
func runGatherPRContext(args []string, stdout, stderr io.Writer) int {
	...
	if err := checkoutExistingBranch(".", selected.Head, pushToken); err != nil {
...
// new one). Authenticated the same env-var-injected way as push-branch's
// gitPushBranch (#237): never a URL-embedded credential, never persisted to
// disk.
func checkoutExistingBranch(dir, branch, token string) error {
