...
var credentialedCapabilities = []capability.Capability{
	capability.RepoPush, capability.GitHubIssuesWrite, capability.GitHubPRWrite, capability.GitHubBranchDelete, capability.GitHubPRMerge,
}
...
func buildRunnerConfig(l instance.Layout, cfg *instance.Config, goobers map[string]apiv1.GooberSpec, tel *telemetry.Client, sharedReg *journal.RegistryScrubber) (runner.Config, *worktree.Manager, error) {
	...
	rc := runner.Config{
		...
		// Scheduled implementation runs claim their driving items after Start, so
		// resolve those items from the ledger if a terminal gate has no snapshot.
		EscalationItemIDs: func(runID string) ([]string, error) {
			return claimedItemIDsForRun(l, runID)
		},
