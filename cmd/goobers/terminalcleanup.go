...
func retainTerminalRunBranch(l instance.Layout, manager *worktree.Manager, runID string) error {
	...
	captured, err := terminalRecoveryCaptured(l, runID, captureAt)
	if err != nil || captured {
		return err
	}
	var branch string
	...
	found, err := manager.WithExistingBranchCheckout(context.Background(), url, branch, project.Branch, func(path string) error {
		return callback(context.Background(), worktree.CleanupTarget{
			Path: path, WorktreeID: "terminal-branch", OwnerRunID: runID,
			Gaggle: identity.Gaggle, BaseRef: project.Branch,
...
func terminalRecoveryCaptured(l instance.Layout, runID string, captureAt time.Time) (bool, error) {
	...
	for _, entry := range entries {
		if entry.Record.RunID == runID && entry.Record.CreatedAt.Equal(captureAt) {
			return true, nil
		}
	}
	return false, nil
