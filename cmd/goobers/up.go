...
func runUpContext(parentCtx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	// known (issue #127).
	resumed, warned, err := resumeInterruptedRunsWithRunners(ctx, l, setup.Runners, setup.LegacyRunner, setup.RunnerRegistry, setup.Machines, setup.GooberDigests, setup.RepoRefs, setup.InstanceLog, setup.Telemetry, setup.RollupDB, setup.ReadModel, sched.ReleaseReconciled, &wg)
	if err != nil {
		pf(stderr, "error: %v\n", err)
		return 1
	}
