...
func TestResumeReleasesReconciledSlotForFollowUpTrigger(t *testing.T) {
	...
	resumed, warned, err := resumeInterruptedRuns(ctx, l, setup.Runner, setup.Machines, setup.GooberDigests, setup.RepoRefs, setup.InstanceLog, setup.Telemetry, setup.RollupDB, setup.ReadModel, sched.ReleaseReconciled, &wg)
	if err != nil {
		t.Fatal(err)
	}
	if len(warned) != 0 {
...
func TestResumeJournalsActualPhaseNotHardcodedStatus(t *testing.T) {
	...
	if _, _, err := resumeInterruptedRuns(ctx, l, setup.Runner, setup.Machines, setup.GooberDigests, setup.RepoRefs, setup.InstanceLog, setup.Telemetry, setup.RollupDB, setup.ReadModel, sched.ReleaseReconciled, &wg); err != nil {
		t.Fatal(err)
	}
	wg.Wait()
...
func TestResumePastOrphanedWorktreeAtSameKey(t *testing.T) {
	...
	resumed, warned, err := resumeInterruptedRuns(context.Background(), l, setup.Runner, setup.Machines, setup.GooberDigests, setup.RepoRefs, setup.InstanceLog, setup.Telemetry, setup.RollupDB, setup.ReadModel, sched.ReleaseReconciled, &wg)
	if err != nil {
		t.Fatal(err)
	}
	if len(warned) != 0 {
...
`
	...
	resumed, warned, err := resumeInterruptedRunsWithRunners(
		...
		setup.GooberDigests, setup.RepoRefs, setup.InstanceLog, setup.Telemetry, setup.RollupDB, setup.ReadModel, sched.ReleaseReconciled, &wg,
	)
	if err != nil {
		t.Fatal(err)
	}
