...
func TestStatusDaemonReportsStaleIdentity(t *testing.T) {
	...
	want := "daemon not running: recorded pid 4242 is not running (started 2026-07-16T09:00:00Z); " +
...
func TestStatusDaemonDoesNotMistakeStaleIdentityForManualHolder(t *testing.T) {
	...
	if !strings.HasPrefix(stdout, fmt.Sprintf("daemon not running: recorded pid %d is not running (started ", os.Getpid())) {
		t.Fatalf("stdout = %q", stdout)
	}
}
