...
func (c *updateChecker) once(ctx context.Context) (updateCheckResult, bool) {
	...
	c.warned = false
	// Publish the CONDITION before deciding whether to announce. These are
	// deliberately different lifetimes: the announcement fires once per
	// distinct version, while the condition holds for as long as the build is
	// behind, and clears the moment it is not. Setting it only when announcing
	// would drop the heartbeat clause on the second tick, and leaving it set
	// would keep claiming an update after the operator installed one.
	c.setPending(result)
	// Cache for `goobers status` so a status call renders the same answer
	// without making a request of its own.
	//
	// The condition above is published FIRST and deliberately survives a
	// failed write. The two surfaces then disagree — the heartbeat reports the
	// newer release while `goobers status` still reads the previous cache,
	// which may say the build is current — so the warning names that
	// divergence rather than reporting a bare write error. The alternative
	// ordering trades the wrong way: publishing only after a successful write
	// would drop the ACCURATE surface whenever the cache is unwritable,
	// leaving an operator with a stale `status` and no heartbeat clause at
	// all.
	if err := selfupdate.WriteCheck(c.root, result); err != nil {
		c.warned = true
		return updateCheckResult{warning: fmt.Sprintf(
			"update check cache: %v; the heartbeat is current but `goobers status` will report a stale result until this write succeeds", err)}, true
	}
...
// setPending publishes the heartbeat condition for a completed check.
func (c *updateChecker) setPending(result selfupdate.CheckResult) {
	if c.pending == nil {
		return
	}
