...
_goobers_completion()
{
    ...
    if (( COMP_CWORD == 1 )); then
        candidates="version versions init preflight onboarding examples scaffold agent-kit validate lint fix doctor config speech up down apply self-update service worker dashboard run signal workflow runs status stats features schema explain reset-rate-limit blocked claims trace escalations completion telemetry journal backlog-dedupe backlog-assignment backlog-health backlog-query reconcile-branches push-branch check-fail-first open-pr report-pr-status gate-removal-guard issue-close-out pr-comment-watch set-milestone merge-pr record-merge-refusal merge-queue-poll reconcile-post-merge post-merge telemetry-query docs-churn ios-simulator-test pr-select gather-sibling-context gather-implement-context apply-verdict elect-lander update-behind-pr gather-pr-context gather-review-threads gather-issue-context gather-ci-failures rebase-pr remediation-checkpoint push-remediated respond-to-findings help --version -h --help"
