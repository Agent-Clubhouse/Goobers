...
func seedCircuitBreakerClaim(t *testing.T, l instance.Layout, itemID, runID string) {
	...
	// #4417: the circuit breaker now resolves each claimed item's own
	// recorded repository identity rather than a caller-supplied one, so
	// every fixture that seeds a claim must also record it.
	seedItemRepositoryForTest(t, l, runID, itemID, circuitBreakerTestRepo())
}
