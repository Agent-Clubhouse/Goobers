...
var synopsisByID = map[string]string{
	...
	"init":                   "  goobers init [--allow-ephemeral] [--guided [--instance-path <dir>] [--port=<port|auto>] [--no-open] [--dev-assets=<dir>] [--workdir=<dir>] | --demo [--insecure] | --template=quickstart [--harness <name>] [--source-tree <path> [--json]] | --template=standard --ci-command <JSON-argv> --required-capabilities <list> [--harness <name>]] [path]\n                                scaffold an instance, open guided setup with placement choices, or create a checked-in config source\n",
	...
	"run":                    "  goobers run [--gaggle <name>] [--github-progress] [--pr <number>] [--api <url>] [--request-id <id>] <workflow> [--no-wait] [path]\n                                trigger a run manually (still honors run conditions)\n",
	"run abort":              "  goobers run abort [--api <url>] <run-id> [path]  mark a stuck non-terminal run aborted\n",
	"run cancel":             "  goobers run cancel [--api <url>] <run-id> [path]  cancel a live in-flight run via the daemon\n",
