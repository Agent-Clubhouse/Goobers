...
var synopsisByID = map[string]string{
	...
	"lint":                   "  goobers lint [--json] [flags] [path]   lint config via the single authoritative validation engine (alias for validate)\n",
	...
	"run approve":            "  goobers run approve <run-id> <stage> [path]  approve an escalated gate (not yet implemented, #469)\n",
	"run override":           "  goobers run override <run-id> <stage> [path]  force-pass a nondeterministic gate (not yet implemented, #469)\n",
	"run rerun":              "  goobers run rerun <run-id> <stage> [path]  rerun a stage with an instruction addendum (not yet implemented, #469)\n",
	...
	"mcp-io":                 "  goobers mcp-io                         run the generic goobers-io MCP server over stdio (harness-spawned, not run by hand)\n",
}
