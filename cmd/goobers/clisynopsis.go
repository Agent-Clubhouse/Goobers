...
var synopsisByID = map[string]string{
	...
	"engine-start":           "  goobers engine-start [--gaggle <name>] [--temporal-hostport h:p] [--temporal-namespace <name>] [--task-queue <name>] [--dedupe-key <key>] [--live-journal] <workflow> [path]\n                                dispatch one run onto the tier-3 engine (experimental)\n",
	...
	"worker":                 "  goobers worker [--task-queue <q>]... [--temporal-hostport h:p] [--temporal-namespace <name>] [--drain-timeout <dur>] [--work-root <path>] [--instance <path>] [--blob-store <path>] [--daemon-api <url>] [--dispatch-namespace <ns>]\n                                host a Temporal engine worker (tier-3, experimental)\n",
	...
	"e2e kill-inject":        "  goobers e2e kill-inject --daemon-url <url> --run <run-id> --stage <name> --stage-class <builtin|agentic|local-ci> --namespace <ns> [--token <bearer>] [--poll-timeout <duration>] [--out <record.json>]\n                                perform one live S6 kill-matrix cell (pod-kill) against a real cluster\n",
