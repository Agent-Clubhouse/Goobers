...
var synopsisByID = map[string]string{
	"work-items":             "  goobers work-items [--provider=<name>] [--repository=<owner/name>] [--kind=<pr|issue>] [--id=<id>] [--limit=<n>] [--json] [--rebuild] [path]  list externally actioned pull requests and issues\n",
	...
	"lint":                   "  goobers lint [--json] [--github-annotations] [--check-harness] [--check-repos] [--check-dispatch-namespaces] [--source-tree [--instance <path>]] [--strict] [path]\n                                lint config via the single authoritative validation engine (alias for validate)\n",
	"fix":                    "  goobers fix (--to <version> | --instance-schema) [--write] [path]\n                                mechanically migrate workflows to a target dslVersion, or add the schemaVersion a runners: inventory requires\n",
	"doctor":                 "  goobers doctor (--k8s | --repo | --av-exclusions) [--kubeconfig <path>] [--context <name>] [--report text|json] [--oidc-issuer <url>] [--registry <host>] [--egress <host:port,...>] [--temporal-hostport <host:port>] [--temporal-namespace <name>] [--timeout <duration>] [--work-root <dir>] [--overlay-dir <dir>] [--image-runtime docker|podman] [--image-pull-policy always|never] [--image-tools <tool,...>] [--image-ca <root.pem>] [--checks <id,...>] [--apiserver-endpoint <url>]\n                                preflight Kubernetes, compare repository forge policy, or list and verify antivirus exclusions\n",
	...
	"self-update":            "  goobers self-update [--policy manual|on-release|on-main] [--include-prerelease] [--branch <name>] [--target <tag>] [--health-ticks <n>] [--health-timeout <duration>] [path]\n                                stage and request a supervised binary update\n",
	"service":                "  goobers service install|uninstall|stop|start|status|task-* [path]\n                                install and manage SCM or per-user Scheduled Task supervision\n",
	"service task-install":   "  goobers service task-install [path]\n                                install a per-user Windows Scheduled Task supervisor\n",
	"service task-uninstall": "  goobers service task-uninstall [path]\n                                remove the per-user Windows Scheduled Task supervisor\n",
	"service task-start":     "  goobers service task-start [path]\n                                start the per-user Windows Scheduled Task supervisor\n",
	"service task-stop":      "  goobers service task-stop [path]\n                                stop the per-user Windows Scheduled Task supervisor\n",
	"service task-status":    "  goobers service task-status [--json] [path]\n                                report per-user Windows Scheduled Task state and identity\n",
	...
	"run":                    "  goobers run [--force] [--gaggle <name>] [--github-progress] [--pr <number>] [--api <url> | --no-api] [--api-timeout <duration>] [--request-id <id>] <workflow> [--no-wait] [path]\n                                trigger a run manually; --force bypasses cadence budgets\n",
	"run abort":              "  goobers run abort [--api <url>] <run-id> [path]  mark a stuck non-terminal run aborted\n",
	"run cancel":             "  goobers run cancel [--api <url> | --no-api] [--request-id <id>] <run-id> [path]  cancel a live in-flight run via the daemon\n",
	...
	"telemetry":              "  goobers telemetry stats|merges|errors|export|mark-fix|prune|prune-orphans|compact [flags] [path]\n                                query, export, mark fixes, prune, or compact run telemetry\n",
	...
	"backlog-query":          "  goobers backlog-query [--debug] [--read-only | --claim [--resweep] | --reconcile | --release]  query/claim one eligible backlog item (a workflow stage)\n",
	...
	"security-alerts-query":  "  goobers security-alerts-query --source code-scanning|dependabot [--state <state>] [--severity <list>] [--tool <name>] [--ref <ref>] [--ecosystem <list>] [--scope runtime|development] [--max-results <n>]\n                                emit bounded, untrusted security alerts for work nomination (a connector stage)\n",
