...
Every key in "outputs" — whatever name you give it — must be a bare string, number, boolean, or null. Never an array or object, for any key: outputs accepts scalars only, and a structured value is schema-rejected. Larger or structured data belongs in "artifacts" instead. For example, on a "blocked" status, if you can name specific blocking issue numbers, set outputs.blockedBy to a single comma-separated string (e.g. "441,442") — never an array or object.
