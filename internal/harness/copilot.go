...
// scanBalancedJSONSpan scans payload from start — which must index an opening
// '{' or '[' — for the matching closer at depth zero, honouring string
// literals and escapes so a brace or bracket inside a string does not corrupt
// the depth count. It reports the exclusive end offset of the balanced span,
// or ok=false if payload runs out before the opener's depth returns to zero.
// Shared by nextJSONValue and firstJSONValue, which differ only in what they
// do with an opener whose balanced span is not valid JSON.
...
func nextJSONValue(payload []byte) (value []byte, start, end int, ok bool) {
	...
	for {
		...
		// Balanced but not valid JSON: skip this opener and resume the search
		// after it.
		searchFrom = start + 1
	}
}
