...
// get/put + surrender put + telemetry read): their token proves "I am run X's
// stage pod", which authorizes ledger operations, credential resolution,
// journal emission, blob transfer, result surrender, and its own gaggle's
// derived telemetry for that run and nothing else. Per-run containment (the
// request body's runId, or the path run id for the journal and surrender
// planes, matching the pod's run) is enforced by the plane handlers, which
// can see the request — the credential and blob handlers additionally refuse
// human principals outright (DS9: those planes serve stage pods only; the
// blob plane's digest carries no run to compare against). The telemetry read
// is contained by GAGGLE rather than by run, because the evidence it serves
// is a gaggle-wide rollup by construction; that check also lives in the
...
func RequireRoles() Authorizer {
	return authorizerFunc(func(request *http.Request) error {
		...
		if IsPodPrincipal(principal) {
			...
			return fmt.Errorf("pod principal %q may only call the claims, credential, journal, blob, surrender, and telemetry-read planes", principal.Subject)
		}
		required := RoleView
