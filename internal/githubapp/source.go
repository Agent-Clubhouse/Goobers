...
// resolved through the same machinery as any token ref, per mint — including a
// store-backed ref (#683), so the App key may itself live in a secret store;
// stores may be nil only when the key ref is env/file, otherwise construction
// fails closed.
func Source(repo instance.RepoRef, registrar SecretRegistrar, stores credentials.StoreResolver) (*TokenSource, error) {
	...
	keyResolver, err := credentials.NewResolverWith([]credentials.TokenRef{repo.Auth.PrivateKey.CredentialTokenRef(refName)}, stores, nil)
	if err != nil {
