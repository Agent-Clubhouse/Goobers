...
func TestBacklogCurationDryRun(t *testing.T) {
	...
	if gotQueryInputs["excludeLabels"] != "goobers:ready,goobers:needs-human,goobers:needs-remediation" {
		t.Errorf("query-backlog excludeLabels input = %v, want terminal output markers only", gotQueryInputs["excludeLabels"])
	}
