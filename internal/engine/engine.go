...
const (
	...
	// stageScheduleToStart bounds how long a stage may sit unclaimed on its
	// task queue before the run fails. It exists for the mixed-platform case:
	// a stage routed to "<queue>-windows" with no Windows worker polling would
	// otherwise wait indefinitely, because Temporal's default here is
	// unlimited. Generous enough that a scale-from-zero node pool (a cold
	// Windows node pulls a multi-GB image) still wins the race, short enough
	// that a genuinely absent worker is reported rather than hung.
...
// capability, per docs/design/mixed-platform-cloud-nodes.md: a stage naming
// os=<goos> is polled from "<workflow queue>-<goos>", and anything else
// inherits the workflow's own queue. Unlabelled therefore means linux, which is
// the documented default, and an all-linux instance needs no extra queues.
//
...
func stageActivityOptions(limits apiv1.Limits, taskQueue string) workflow.ActivityOptions {
	...
	return workflow.ActivityOptions{
		// Empty inherits the workflow's queue — the linux default.
		TaskQueue:           taskQueue,
		StartToCloseTimeout: timeout,
		// FAIL FAST when nothing is polling the target queue. Temporal's
		// documented default for ScheduleToStartTimeout is unlimited, so a
		// stage routed to a platform with no capacity would queue FOREVER
		// rather than surfacing — the opposite of the fail-fast behaviour
		// docs/design/mixed-platform-cloud-nodes.md promises, and impossible
		// to distinguish from a slow stage while it happens.
