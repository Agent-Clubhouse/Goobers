...
func TestTaskExecutor_CIPollRejectsGitHubCapabilityForADO(t *testing.T) {
	shell, _ := newTestExecutor(t, nil)
	te := newRegisteredTaskExecutor(t, shell, nil)
	env := apiv1.InvocationEnvelope{
		RepoRef:      apiv1.RepoRef{Provider: apiv1.ProviderADO, Owner: "acme", Project: "widgets", Name: "web"},
		Capabilities: []string{string(capability.GitHubPRWrite)},
		Inputs:       map[string]interface{}{InputKind: KindCIPoll, InputPRNumber: "9"},
	}
	if _, err := te.Run(context.Background(), env, apiv1.DeterministicRun{}); err == nil ||
		!strings.Contains(err.Error(), `requires declared capability "provider:pr:write"`) {
		t.Fatalf("Run error = %v, want provider-neutral capability error", err)
	}
}
