...
func TestBuildExcludesTestModulesButPreservesRuntimeAssets(t *testing.T) {
	source := fstest.MapFS{
		sourceRoot + "/extension.mjs":          {Data: []byte("export {};\n")},
		sourceRoot + "/module.mjs":             {Data: []byte("export const x = 1;\n")},
		sourceRoot + "/module.test.mjs":        {Data: []byte("// test double\n")},
		sourceRoot + "/extension.test.mjs":     {Data: []byte("// test entrypoint\n")},
		sourceRoot + "/goober-mascot.png":      {Data: []byte("png")},
		sourceRoot + "/copilot-extension.json": {Data: []byte(`{"name":"goobers-portal","version":1}`)},
	}
	bundle, err := Build(source, "v1.0.0", "abc123")
	if err != nil {
		t.Fatal(err)
	}
	for name := range bundle.Files {
		if strings.HasSuffix(name, ".test.mjs") {
			t.Fatalf("bundle unexpectedly ships %q", name)
		}
	}
	for _, want := range []string{"extension.mjs", "module.mjs", "goober-mascot.png", "copilot-extension.json"} {
		if _, ok := bundle.Files[want]; !ok {
			t.Fatalf("bundle missing runtime asset %q; files=%v", want, mapKeys(bundle.Files))
		}
	}
	for _, asset := range bundle.Manifest.Assets {
		if strings.HasSuffix(asset.Path, ".test.mjs") {
			t.Fatalf("manifest lists test module %q", asset.Path)
		}
	}
}

func TestInstallExcludesTestModules(t *testing.T) {
	source := fstest.MapFS{
		sourceRoot + "/extension.mjs":          {Data: []byte("export {};\n")},
		sourceRoot + "/module.mjs":             {Data: []byte("export const x = 1;\n")},
		sourceRoot + "/module.test.mjs":        {Data: []byte("// test double\n")},
	...
	if _, err := os.Stat(filepath.Join(manager.Path(), "module.test.mjs")); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("installed extension includes test module: %v", err)
	}
	if _, err := os.Stat(filepath.Join(manager.Path(), "module.mjs")); err != nil {
		t.Fatalf("installed extension missing runtime module: %v", err)
	}
	report, err := manager.Check(bundle)
	if err != nil || report.State != "current" {
		t.Fatalf("Check after install = %+v, %v", report, err)
	}
	// A stray test file dropped into the installed directory is drift, not a filtered
	// shipping asset, so it must still surface as Unexpected in the report.
	stray := filepath.Join(manager.Path(), "extra.test.mjs")
	...
	if report.State != "modified" {
		t.Fatalf("Check with stray file = %q, want modified", report.State)
	}
	found := false
	for _, path := range report.Unexpected {
		if path == "extra.test.mjs" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("Unexpected drift report missing stray file: %+v", report.Unexpected)
	}
}
