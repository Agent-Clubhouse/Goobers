...
func Build(source fs.FS, version, commit string) (Bundle, error) {
	...
	err := fs.WalkDir(source, sourceRoot, func(name string, entry fs.DirEntry, walkErr error) error {
		...
		// Test modules are repository checks, not assets shipped to Copilot.
		if strings.HasSuffix(relative, ".test.mjs") {
			return nil
		}
...
func (m *Manager) Check(bundle Bundle) (Report, error) {
	...
	modified, missing, err := verifyOwnedAssets(report.Path, installed.Assets)
	if err != nil {
		return Report{}, err
	}
	...
	unexpected, err := findUnexpected(report.Path, owned)
	if err != nil {
		return Report{}, err
	}
	report.Unexpected = append(report.Unexpected, unexpected...)
...
// verifyOwnedAssets stats each asset the installed manifest claims and reports
// which are missing or whose on-disk contents no longer match.
func verifyOwnedAssets(reportPath string, assets []Asset) (modified, missing []string, err error) {
	for _, asset := range assets {
		assetPath := filepath.Join(reportPath, filepath.FromSlash(asset.Path))
		...
		if statErr != nil {
			return nil, nil, fmt.Errorf("inspect installed portal extension asset %s: %w", asset.Path, statErr)
		}
		if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() {
			modified = append(modified, asset.Path)
			continue
		}
		data, readErr := os.ReadFile(assetPath)
		if readErr != nil {
			return nil, nil, fmt.Errorf("read installed portal extension asset %s: %w", asset.Path, readErr)
		}
		sum := sha256.Sum256(data)
...
// findUnexpected walks the installed directory and returns files not claimed
// by the manifest, skipping the manifest file itself and the legacy state
// directory.
func findUnexpected(reportPath string, owned map[string]string) ([]string, error) {
	var unexpected []string
	err := filepath.WalkDir(reportPath, func(name string, entry fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		relative, err := filepath.Rel(reportPath, name)
		if err != nil || relative == "." {
			return err
		}
	...
	if err != nil {
		return nil, fmt.Errorf("inspect installed portal extension: %w", err)
	}
...
func (m *Manager) replace(bundle Bundle, existing bool) error {
	...
	stage, err := stageBundle(bundle, extensions)
	if err != nil {
		return err
	}
	...
	return m.commitUpdate(bundle, stage)
}

// stageBundle writes the bundle's files and manifest into a fresh temp dir
// under the Copilot extensions directory and returns the staging path. The
// caller is responsible for removing the returned path.
func stageBundle(bundle Bundle, extensions string) (string, error) {
	stage, err := os.MkdirTemp(extensions, "."+Name+"-install-")
	if err != nil {
		return "", fmt.Errorf("create portal extension staging directory: %w", err)
	}
	if err := writeBundleFiles(bundle, stage); err != nil {
...
func writeBundleFiles(bundle Bundle, stage string) error {
...
// commitUpdate replaces an existing installation with the staged bundle,
// keeping the previous copy at ".previous" for rollback recovery.
func (m *Manager) commitUpdate(bundle Bundle, stage string) error {
