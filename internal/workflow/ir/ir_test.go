...
func TestNormalizeReturnsImmutableSnapshot(t *testing.T) {
	def := workflow.Definition{
		...
		Annotations: map[string]string{
			"goobers.dev/allow-preview-features": "yes",
		},
		Spec: apiv1.WorkflowSpec{
			Gaggle: "g", Start: "task",
			Triggers: []apiv1.Trigger{{
	...
	def.Annotations["goobers.dev/allow-preview-features"] = "no"
	...
	if doc.Triggers[0].Selector["label"] != "ready" ||
		doc.SourceDefinition.Annotations["goobers.dev/allow-preview-features"] != "yes" ||
