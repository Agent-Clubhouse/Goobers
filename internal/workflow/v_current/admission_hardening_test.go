...
// inventoried subcommands. v_next/testdata/golden/runtime-policy.yaml is the
...
func TestCompileScratchWorkspaceCarriesNoRepoStateAcrossPlatforms(t *testing.T) {
	...
	// (No repo-readonly variant here: DSL 1.4 has no task.workspace at all —
	// setting it is its own rejection, TestCompileRejectsTaskWorkspace.)
}

// --- Unknown-to-1.4 constructs become errors (Part 3, PO-ratified) -------

func TestCompileRejectsParallelsBlock(t *testing.T) {
	spec := linearSpec()
	spec.Parallels = []apiv1.Parallel{{Name: "fan-out"}}
	_, err := compileAcknowledged(Definition{Name: "parallels-1-4", Version: 1, Spec: spec})
	want := `parallel "fan-out": parallels is not part of DSL 1.4; available in 2.0`
	if err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("Compile error = %v, want containing %q (#2738: the block used to be silently dropped)", err, want)
	}

	// Unchanged: the same workflow without the block still compiles.
	if _, err := compileAcknowledged(Definition{Name: "no-parallels", Version: 1, Spec: linearSpec()}); err != nil {
		t.Fatalf("workflow without parallels must keep compiling: %v", err)
	}
}

func TestCompileRejectsTaskWorkspace(t *testing.T) {
	spec := linearSpec()
	spec.Tasks[0].Workspace = apiv1.WorkspaceRepoReadOnly
	_, err := compileAcknowledged(Definition{Name: "task-workspace-1-4", Version: 1, Spec: spec})
	want := `task "implement": task.workspace is not part of DSL 1.4; available in 2.0`
	if err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("Compile error = %v, want containing %q (the author asked for repo-readonly and silently got WRITABLE)", err, want)
	}

	// Unchanged: run.workspace (the 1.4 spelling for deterministic tasks)
	// still works.
	task := apiv1.Task{
		Name: "collect",
		Type: apiv1.TaskDeterministic,
	...
	if _, err := compileAcknowledged(Definition{Name: "run-workspace", Version: 1, Spec: singleTaskSpec(task)}); err != nil {
		t.Fatalf("run.workspace must keep compiling in 1.4: %v", err)
	}
}
