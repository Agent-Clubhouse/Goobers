...
// rejected (a non-shell inputs.kind exempts its placeholder command); a
// provably cross-platform task transition handing off unpushed repo
// workspace state is rejected (#2861, see pushBoundaryProblems); and
// constructs this version never implemented (`parallels:`, task.workspace)
// are rejected instead of silently dropped (see
// unknownVersionConstructProblems). Errors are aggregated so one compile
...
func Compile(def Definition, opts ...Option) (*Machine, error) {
	...
	problems = append(problems, unknownVersionConstructProblems(def)...)
...
// unknownVersionConstructProblems rejects constructs this version does not
// implement instead of silently dropping them — the breaking-rejections wave
// of the 1.4 lock (PO-ratified: silent drops become loud before the v0.2.0
// tags), following runScriptProblems' existing pattern.
//
//   - A `parallels:` block compiled clean here but never executed (#2738):
//     newMachine builds no parallel states, so every parallel silently
//     vanished from the machine.
//   - `task.workspace` was ignored entirely: this version's runner contract
//     reads only Run.Workspace, so an author asking for repo-readonly
//     silently got the WRITABLE repo default.
func unknownVersionConstructProblems(def Definition) []string {
	var problems []string
	for _, parallel := range def.Spec.Parallels {
		problems = append(problems, fmt.Sprintf(
			"parallel %q: parallels is not part of DSL %s; available in %s",
			parallel.Name, DSLVersion, "2.0",
		))
	}
	for _, task := range def.Spec.Tasks {
		if task.Workspace != "" {
			problems = append(problems, fmt.Sprintf(
				"task %q: task.workspace is not part of DSL %s; available in %s",
				task.Name, DSLVersion, "2.0",
			))
		}
	}
...
func newMachine(def Definition) (*Machine, error) {
	...
	// report nothing for them. A `parallels:` block is a compile ERROR
	// (unknownVersionConstructProblems, #2738) — it used to be silently
	// dropped here, compiling a machine that never executed the block.
...
// t with no intermediate stage. Terminals and reserved targets end the walk.
// The result is deterministic: discovery order with gate branches visited in
