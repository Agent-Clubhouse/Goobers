...
// checkFrozenPatchLog is DVL-9's (#869) CI guard, pure so it's directly
// testable against synthetic inputs (TestCheckFrozenPatchLog below) as well
// as the real golden fixtures (TestFrozenGoldenChangeRequiresAcknowledgedPatch).
//
// v_current is frozen (its own README, and design doc §3.5/DVL-9) — its
// compiled-digest goldens may only ever move via a reviewed, contract-
// preserving interpreter PATCH, never silently. TestGoldenCompiledSemanticDigests
// already fails on an ACCIDENTAL digest drift (compiled output no longer
// matches the committed digests.json); this closes the other half: a
// DELIBERATE digests.json edit must be accompanied by a human-authored
// PATCH_LOG.json entry, or CI stays red. digestsSha256 pins digests.json's
// own committed bytes, so bumping it (the only way to make the guard pass
// again after a real digests.json change) forces the same PR to also touch
// PATCH_LOG.json's patches list — there is no path to a green frozen-golden
// change that skips recording why.
	...
	if log.DigestsSha256 != gotSha {
		return fmt.Errorf(
			"digests.json changed (sha256 %s) without an acknowledged patch (PATCH_LOG.json pins %s) — "+
				"v_current is frozen (design doc §3.5, DVL-9/#869): a contract-preserving patch must bump "+
...
func TestCheckFrozenPatchLogCatchesDrift(t *testing.T) {
	validLog := `{"digestsSha256":"%s","patches":[{"version":1,"binaryVersion":"v0.1.0","description":"initial freeze"}]}`
	...
	t.Run("empty description on the latest entry fails", func(t *testing.T) {
		log := fmt.Sprintf(`{"digestsSha256":"%s","patches":[{"version":1,"binaryVersion":"v0.1.0","description":""}]}`, shaOf(digestsA))
		if err := checkFrozenPatchLog(digestsA, []byte(log)); err == nil {
	...
	t.Run("non-dense version numbering fails", func(t *testing.T) {
		log := fmt.Sprintf(`{"digestsSha256":"%s","patches":[{"version":3,"binaryVersion":"v0.1.0","description":"fix"}]}`, shaOf(digestsA))
		if err := checkFrozenPatchLog(digestsA, []byte(log)); err == nil {
...
// reviewed patch applied to this frozen interpreter, keyed to the exact
