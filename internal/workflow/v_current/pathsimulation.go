...
// exactly as a run would: one gate outcome at a time, carrying forward a
// symbolic model of what the last task actually emitted, so a handoff that
// only breaks on a specific path is caught with that exact path as evidence.
//
