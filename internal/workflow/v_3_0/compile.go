...
// interpreter's own DSL version (the D7 prerequisite that landed as #3522):
// a requirement change gated to a later DSL version is invisible here, and a
// 3.0-gated change (sinceDSL "3.0") is invisible to the frozen 2.0
// interpreter — so manifest edits can never re-break shipped configs of
// another version (the ed11ae81 class, #3504).
...
// (GaggleSpec.RunsOn, dsl-3.0.md §2) so compilation evaluates each stage's
// EFFECTIVE runsOn — gaggle capabilities/restrictions union the stage's own,
// and a gaggle-vs-stage OS conflict is a compile error, never a silent
// override. Callers that hold the gaggle must pass it; nil evaluates
// stage-level runsOn alone.
func WithGaggleRunsOn(runsOn *apiv1.GaggleRunsOn) Option {
	return func(o *options) { o.gaggleRunsOn = runsOn }
}
...
// rejected (a non-shell inputs.kind exempts its placeholder command).
//
// DSL 3.0 additionally rejects: surface that no longer exists in 3.0
...
func claimLedgerPlacementProblems(def Definition) []string {
	var firstTask, firstPlacement string
	for _, task := range def.Spec.Tasks {
		...
		placement := canonicalPlacement(task.RunsOn)
		if firstTask == "" {
			firstTask = task.Name
		...
		if placement != firstPlacement {
			return []string{fmt.Sprintf(
				"claims-mutating tasks %q and %q declare incompatible runsOn placement (%s vs %s); all claims-mutating tasks must use identical placement requirements",
...
// canonicalPlacement renders a stage's runsOn block into one canonical,
// comparable string — the 3.0 analog of 2.0's sorted requiredCapabilities
// list. Every placement-relevant field participates: two claims-mutating
// stages that could match different runner sets must not diverge, because the
// claim ledger is host-local state (#2861's claims-mutating corollary).
func canonicalPlacement(runsOn *apiv1.RunsOn) string {
	if runsOn == nil {
		return "os= cpu= memory= disk= caps=[] restrictions=[]"
	}
	return fmt.Sprintf("os=%s cpu=%s memory=%s disk=%s caps=%v restrictions=%v",
		runsOn.OS, runsOn.CPU, runsOn.Memory, runsOn.Disk,
		sortedDistinct(runsOn.Capabilities), sortedDistinct(runsOn.Restrictions))
}

// sortedDistinct returns a sorted, de-duplicated copy of values.
...
func requiresModelCapability(h apiv1.Harness, knownHarnesses map[string]bool) bool {
	return knownHarnesses != nil && (h == apiv1.HarnessCopilot || h == apiv1.HarnessClaudeCode || h == apiv1.HarnessCodex)
}
