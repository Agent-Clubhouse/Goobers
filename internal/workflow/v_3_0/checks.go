...
// CheckPushBoundaries is a no-op in DSL 3.0: the #2861 cross-platform
// push-boundary rule is subsumed by declared repo-handoff edges (dsl-3.0.md
// §4/§7) — with every chain declared and transport runtime-owned at declared
// edges, the unsafe transition the 2.0 rule rejected cannot be written. The
// signature is kept for the versioned-interpreter table.
func CheckPushBoundaries(Definition, []string) []string { return nil }

// CheckRunsOnOSTokens reports every os=* capability token in the document —
// CAP004 (dsl-3.0.md D12): a 3.0 document has exactly one platform
// vocabulary, runsOn.os. gaggleRunsOn is the workflow's gaggle-level floor;
// nil checks stage-level declarations alone.
func CheckRunsOnOSTokens(def Definition, gaggleRunsOn *apiv1.GaggleRunsOn) []string {
	return osTokenProblems(def, gaggleRunsOn)
}

// CheckRunsOnRestrictions reports every restriction token outside the closed
// v1 effect list, with a did-you-mean suggestion — CAP005 (dsl-3.0.md §5).
func CheckRunsOnRestrictions(def Definition, gaggleRunsOn *apiv1.GaggleRunsOn) []string {
	return restrictionProblems(def, gaggleRunsOn)
}

// CheckRunsOnPlacement reports structural runsOn problems — an invalid os
// enum value, a malformed quantity, a gaggle-vs-stage OS conflict — plus the
// removed-in-3.0 surface refusals (requiredCapabilities, run.network,
// inherited 2.0 spellings) with pointer diagnostics.
func CheckRunsOnPlacement(def Definition, gaggleRunsOn *apiv1.GaggleRunsOn) []string {
	problems := removedSurfaceProblems(def)
	return append(problems, runsOnProblems(def, gaggleRunsOn)...)
}
