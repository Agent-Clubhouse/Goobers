...
const (
	...
	// scheduling block, the gaggle-level floor, declared repo-handoff edges,
	// and the committing-script producer opt-in. These IDs exist only in the
	// 3.0 registry; the removed 2.0 surface they supersede
	// (task.requiredCapabilities, gaggle.spec.requiredCapabilities,
	// gaggle.spec.sandbox, stage.run.network.none) deliberately has no ID
	// here — the interpreter refuses those fields with pointer diagnostics.
	featureTaskRunsOn               FeatureID = "task.runsOn"
	featureTaskRunsOnOS             FeatureID = "task.runsOn.os"
	featureTaskRunsOnCPU            FeatureID = "task.runsOn.cpu"
	featureTaskRunsOnMemory         FeatureID = "task.runsOn.memory"
	featureTaskRunsOnDisk           FeatureID = "task.runsOn.disk"
	featureTaskRunsOnCapabilities   FeatureID = "task.runsOn.capabilities"
	featureTaskRunsOnRestrictions   FeatureID = "task.runsOn.restrictions"
	featureTaskRepoFrom             FeatureID = "task.repoFrom"
	featureTaskCommitsRepo          FeatureID = "task.commitsRepo"
	featureGaggleRunsOn             FeatureID = "gaggle.spec.runsOn"
...
func currentFeatures(sinceVersion string) []Feature {
	ids := []FeatureID{
		...
		featureTaskRunsOn,
		featureTaskRunsOnOS,
		featureTaskRunsOnCPU,
		featureTaskRunsOnMemory,
		featureTaskRunsOnDisk,
		featureTaskRunsOnCapabilities,
		featureTaskRunsOnRestrictions,
		featureTaskRepoFrom,
		featureTaskCommitsRepo,
		featureGaggleRunsOn,
	...
	for _, id := range ids {
		...
		since := sinceVersion
		if introduced, isNew := v30Introductions[id]; isNew {
			since = introduced
		}
		level := SupportGA
...
// v30Introductions records the app release line that introduces each
// 3.0-only feature — the same release the DSL 3.0 preview entry names in the
// support matrix (internal/supportmatrix). Carried-forward surface keeps its
// historical since-version; only genuinely new surface starts here. The
// features register GA within 3.0: the version-level PREVIEW gate
// (DVL010/DVL011) is the opt-in that guards the whole surface, and stacking a
// per-feature preview gate on top would demand two acknowledgements for one
// decision.
const v30InitialFeatureVersion = "v0.4.0"

var v30Introductions = map[FeatureID]string{
	featureTaskRunsOn:               v30InitialFeatureVersion,
	featureTaskRunsOnOS:             v30InitialFeatureVersion,
	featureTaskRunsOnCPU:            v30InitialFeatureVersion,
	featureTaskRunsOnMemory:         v30InitialFeatureVersion,
	featureTaskRunsOnDisk:           v30InitialFeatureVersion,
	featureTaskRunsOnCapabilities:   v30InitialFeatureVersion,
	featureTaskRunsOnRestrictions:   v30InitialFeatureVersion,
	featureTaskRepoFrom:             v30InitialFeatureVersion,
	featureTaskCommitsRepo:          v30InitialFeatureVersion,
	featureGaggleRunsOn:             v30InitialFeatureVersion,
	featureGaggleRunsOnOS:           v30InitialFeatureVersion,
	featureGaggleRunsOnCapabilities: v30InitialFeatureVersion,
	featureGaggleRunsOnRestrictions: v30InitialFeatureVersion,
	featureGateRunsOn:               v30InitialFeatureVersion,
	featureGateRunsOnOS:             v30InitialFeatureVersion,
	featureGateRunsOnCPU:            v30InitialFeatureVersion,
	featureGateRunsOnMemory:         v30InitialFeatureVersion,
	featureGateRunsOnDisk:           v30InitialFeatureVersion,
	featureGateRunsOnCapabilities:   v30InitialFeatureVersion,
	featureGateRunsOnRestrictions:   v30InitialFeatureVersion,
	featureTaskRunsOnWindowsAdmin:   v30InitialFeatureVersion,
	featureGateRunsOnWindowsAdmin:   v30InitialFeatureVersion,
	featureGaggleRunsOnWindowsAdmin: v30InitialFeatureVersion,
}
...
// gated behind the explicit goobers.dev/allow-preview-features opt-in. The
// 3.0 registry has none: DSL 3.0 is itself a PREVIEW version, so the entire
// surface sits behind the version-level DVL010/DVL011 opt-in already, and the
// sole remaining 2.0 preview feature (the per-gaggle sandbox override,
// featureGaggleSandbox) does not carry into 3.0 as itself — it folds into the
// restrictions model (dsl-3.0.md D15; FeaturesForGaggle refuses the field
// with a pointer diagnostic). When external call-out's typed run.kind surface
// (#2976, external-call-out-stages.md) lands in the shared schema, it
// registers GA here — not preview — per D15's "promotes to 3.0 GA".
...
//
// Two gaggle fields do not exist in DSL 3.0 and are refused with pointer
// diagnostics rather than a bare registry miss:
//
//   - spec.sandbox — the per-gaggle sandbox override folds into the
//     restrictions model (dsl-3.0.md D15/PO-D7). The full migration mapping
//     into runsOn.restrictions is the migrator's work (#3516); until it
//     lands, 3.0 refuses the field so the posture can never be silently
//     dropped.
//   - spec.requiredCapabilities — superseded by spec.runsOn (D1/D12).
func FeaturesForGaggle(spec apiv1.GaggleSpec) ([]Feature, error) {
	if spec.Sandbox != nil {
		return nil, fmt.Errorf(
			"gaggle spec.sandbox does not exist in DSL 3.0: the runsOn.restrictions model subsumes the per-gaggle sandbox override (dsl-3.0.md D15) — remove the block, or keep this gaggle's workflows on DSL 2.0 until `goobers fix --to 3.0` (#3516) maps the posture onto restrictions")
	}
	if spec.RequiredCapabilities != nil {
		return nil, fmt.Errorf(
			"gaggle spec.requiredCapabilities does not exist in DSL 3.0: move the tokens to spec.runsOn.capabilities and any os=* token to spec.runsOn.os (`goobers fix --to 3.0` performs this rewrite)")
	}
	used := featureSet{}
	if spec.RunsOn != nil {
		used.add(featureGaggleRunsOn)
		if spec.RunsOn.OS != "" {
			used.add(featureGaggleRunsOnOS)
		}
...
func FeaturesForGoober(spec apiv1.GooberSpec) ([]Feature, error) {
	...
	if spec.Harness == apiv1.HarnessCodex {
		used.add(featureGooberHarnessCodex)
	}
	if spec.Model != "" {
...
func addTaskFeatures(used featureSet, task apiv1.Task) {
	...
	if task.RunsOn != nil {
		used.add(featureTaskRunsOn)
		if task.RunsOn.OS != "" {
			used.add(featureTaskRunsOnOS)
		}
		if task.RunsOn.CPU != "" {
			used.add(featureTaskRunsOnCPU)
		}
		if task.RunsOn.Memory != "" {
			used.add(featureTaskRunsOnMemory)
		}
		if task.RunsOn.Disk != "" {
			used.add(featureTaskRunsOnDisk)
		}
	...
	if task.RepoFrom != nil {
		used.add(featureTaskRepoFrom)
	}
	if task.CommitsRepo {
		used.add(featureTaskCommitsRepo)
	}
	if task.Outbox != nil {
	...
	// task.run.network has no ID in the 3.0 registry: run.network folds into
	// runsOn.restrictions (dsl-3.0.md D16) and removedSurfaceProblems refuses
	// the field with a pointer diagnostic.
