...
var schemaLeafExceptions = map[string]string{
	...
	// Surface that exists in the shared schemas for earlier DSL versions but
	// not in DSL 3.0 (dsl-3.0.md D1/D15/D16): the schemas are shared across
	// interpreters, so these leaves stay parseable while this interpreter
	// refuses them with pointer diagnostics (removedSurfaceProblems /
	// FeaturesForGaggle) instead of registering them.
	"gaggle.spec.sandbox.agentic":              "does not exist in DSL 3.0: folds into the restrictions model (D15); FeaturesForGaggle refuses the field",
	"gaggle.spec.requiredCapabilities":         "does not exist in DSL 3.0: superseded by gaggle.spec.runsOn (D1/D12); FeaturesForGaggle refuses the field",
	"workflow.spec.tasks.requiredCapabilities": "does not exist in DSL 3.0: superseded by task.runsOn.capabilities (D1/D12); removedSurfaceProblems refuses the field",
	"workflow.spec.tasks.run.network":          "does not exist in DSL 3.0: folds into runsOn.restrictions (D16); removedSurfaceProblems refuses the field",
