	...
	spec := apiv1.WorkflowSpec{
		...
		Tasks: []apiv1.Task{
			...
			},
			{
				...
				RunsOn: &apiv1.RunsOn{OS: "windows"},
			},
		},
	}
	_, err := compileAcknowledged(Definition{Name: "claims-placement", Version: 1, Spec: spec})
	if err == nil || !strings.Contains(err.Error(), `claims-mutating tasks "claim" and "release" declare incompatible runsOn placement`) {
		t.Fatalf("Compile error = %v, want incompatible claims placement", err)
	}
}

func TestClaimLedgerPlacementProblems(t *testing.T) {
	task := func(name, command string, args []string, runsOn *apiv1.RunsOn) apiv1.Task {
		return apiv1.Task{
	...
	tests := []struct {
		...
		{
			name: "same placement in different capability order",
			tasks: []apiv1.Task{
				task("select", "pr-select", nil, &apiv1.RunsOn{OS: "linux", Capabilities: []string{"git", "make"}}),
				task("release", "pr-claim", []string{"--release"}, &apiv1.RunsOn{OS: "linux", Capabilities: []string{"make", "git"}}),
			},
		},
		{
			...
			tasks: []apiv1.Task{
				...
				task("close", "issue-close-out", nil, &apiv1.RunsOn{OS: "linux"}),
			},
			want: true,
		},
		{
			name: "quantity minimums differ",
			tasks: []apiv1.Task{
				task("select", "pr-select", nil, &apiv1.RunsOn{Memory: "4Gi"}),
				task("release", "pr-claim", []string{"--release"}, &apiv1.RunsOn{Memory: "8Gi"}),
			},
			want: true,
		},
		{
			...
			tasks: []apiv1.Task{
				task("read", "backlog-query", []string{"--read-only"}, &apiv1.RunsOn{OS: "windows"}),
				task("claim", "backlog-query", []string{"--claim"}, &apiv1.RunsOn{OS: "linux"}),
			},
		},
	}
