...
type compiledSemantics struct {
	...
	// RepoFromCoverage pins the reaching-definitions result per repo consumer
	// (dsl-3.0.md §4): a digest drift here means the WF022 analysis changed —
	// in particular the discriminator fixture's local-ci coverage
	// [implement, remediate-ci] regressing to the back-edge-pruning answer
	// [implement] is an automatic golden failure (§9 item 10's compile half).
	RepoFromCoverage map[string][]string `json:"repoFromCoverage"`
}
...
type validationSemantics struct {
	...
	// The DSL 3.0 checks (dsl-3.0.md §5): CAP004, CAP005, structural runsOn
	// problems, and WF022 — pinned so a golden fixture that must stay clean
	// (or must keep failing a specific way) cannot drift silently.
	RunsOnOSTokens     []string `json:"runsOnOSTokens,omitempty"`
	RunsOnRestrictions []string `json:"runsOnRestrictions,omitempty"`
	RunsOnPlacement    []string `json:"runsOnPlacement,omitempty"`
	RepoHandoffs       []string `json:"repoHandoffs,omitempty"`
}
...
func semanticDigest(machine *Machine) (string, error) {
	...
	semantics := compiledSemantics{
		...
		Validation: validationSemantics{
			...
			RunsOnOSTokens:         CheckRunsOnOSTokens(machine.Def, nil),
			RunsOnRestrictions:     CheckRunsOnRestrictions(machine.Def, nil),
			RunsOnPlacement:        CheckRunsOnPlacement(machine.Def, nil),
			RepoHandoffs:           CheckRepoHandoffs(machine.Def),
		},
		RepoFromCoverage: RepoFromCoverage(machine.Def),
	}
