...
func TestBacklogProviderForGitHub(t *testing.T) {
	p, repo, err := BacklogProviderFor(apiv1.BacklogRef{Provider: apiv1.ProviderGitHub, Project: "acme/web"}, "tok")
	if err != nil {
...
func TestBacklogProviderForADO(t *testing.T) {
	_, repo, err := BacklogProviderFor(apiv1.BacklogRef{Provider: apiv1.ProviderADO, Project: "myorg/myproject"}, "tok")
	if err != nil {
		t.Fatalf("BacklogProviderFor: %v", err)
	}
...
func TestBacklogProviderForErrors(t *testing.T) {
	if _, _, err := BacklogProviderFor(apiv1.BacklogRef{Provider: apiv1.ProviderGitHub, Project: "noslash"}, "t"); err == nil {
		t.Error("expected error for malformed github project")
	}
	if _, _, err := BacklogProviderFor(apiv1.BacklogRef{Provider: "gitlab", Project: "a/b"}, "t"); err == nil {
