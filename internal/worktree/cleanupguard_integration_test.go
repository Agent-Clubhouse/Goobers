...
func TestIntegrationCleanupHandoffPrecedesRollbackAndDeletion(t *testing.T) {
	...
	for _, mode := range []string{"remove", "keep", "reap", "replace", "finalize"} {
		t.Run(mode, func(t *testing.T) {
			...
			if len(targets) != 1 || targets[0] != (CleanupTarget{Path: wt.Path, WorktreeID: wt.RunID, OwnerRunID: "owner", Gaggle: "issue:42", BaseRef: "refs/heads/main", StartRef: mk.StartRef, RepositoryDigest: mk.RepositoryDigest, CreatedAt: mk.CreatedAt}) || mk.RepositoryDigest == "" || mk.StartRef == "" || mk.CreatedAt.IsZero() {
