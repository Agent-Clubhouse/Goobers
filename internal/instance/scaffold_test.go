...
func TestInitQuickstartFresh(t *testing.T) {
	...
	workflow := set.Workflows[0]
	if workflow.Name != QuickstartTemplate || len(workflow.Spec.Tasks) != 7 || len(workflow.Spec.Gates) != 0 {
	...
	wantTasks := []string{"query-backlog", "implement", "review", "local-ci", "push-branch", "open-pr", "report-cost"}
	for i, task := range workflow.Spec.Tasks {
		if task.Name != wantTasks[i] {
