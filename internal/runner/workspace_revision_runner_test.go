...
func TestRunnerParallelSelectedRepositoryReachesDownstreamInvocation(t *testing.T) {
	const runID = "workspace-revision-parallel-repository"
	revision := runnerWorkspaceRevision("other", "repo", strings.Repeat("a", 40))
	...
	r, _ := newParallelTestRunner(t, func(rec ArtifactRecorder, _ SecretRegistrar) (invoke.Deterministic, error) {
		return &collateCapturingDeterministic{
			delegate: &stubDeterministic{rec: rec, byTask: byTask},
			env:      collateEnv,
		}, nil
	})
	...
	case env := <-collateEnv:
		if env.RepoRef.Owner != "other" || env.RepoRef.Name != "repo" || env.RepoRef.Branch != "main" {
			t.Fatalf("downstream invocation repo ref = %+v, want configured additional repository", env.RepoRef)
		}
	case <-time.After(runnerTestWaitTimeout):
		t.Fatal("post-join invocation was not observed")
	}
}

func TestRunnerParallelSameSelectedRepositoryReachesDownstream(t *testing.T) {
	const runID = "workspace-revision-parallel-same-repository"
	revision := runnerWorkspaceRevision("other", "repo", strings.Repeat("a", 40))
	revision.BaseRepository = nil
	collateEnv := make(chan apiv1.InvocationEnvelope, 1)
	byTask := map[string]stubTaskResult{
		runID + ":lens-a":  {status: apiv1.ResultSuccess, workspaceRevision: revision},
		runID + ":lens-b":  {status: apiv1.ResultSuccess, workspaceRevision: revision},
		runID + ":lens-c":  {status: apiv1.ResultSuccess},
		runID + ":collate": {status: apiv1.ResultSuccess},
	}
	r, _ := newParallelTestRunner(t, func(rec ArtifactRecorder, _ SecretRegistrar) (invoke.Deterministic, error) {
	...
	select {
	case env := <-collateEnv:
		if env.RepoRef.Owner != "other" || env.RepoRef.Name != "repo" {
			t.Fatalf("downstream invocation repo ref = %+v, want shared configured repository", env.RepoRef)
		}
	case <-time.After(runnerTestWaitTimeout):
		t.Fatal("post-join invocation was not observed")
	}
...
func TestRunnerParallelConflictingSelectedRepositoriesFailClosed(t *testing.T) {
	const runID = "workspace-revision-parallel-repository-conflict"
	first := runnerWorkspaceRevision("other", "repo", strings.Repeat("a", 40))
	second := runnerWorkspaceRevision("third", "repo", strings.Repeat("b", 40))
	first.BaseRepository, second.BaseRepository = nil, nil
	byTask := map[string]stubTaskResult{
		runID + ":lens-a":  {status: apiv1.ResultSuccess, workspaceRevision: first},
		runID + ":lens-b":  {status: apiv1.ResultSuccess, workspaceRevision: second},
		runID + ":lens-c":  {status: apiv1.ResultSuccess},
	...
	r.cfg.AdditionalRepos = []apiv1.RepoRef{
		{Provider: apiv1.ProviderGitHub, Owner: "other", Name: "repo", Branch: "main"},
		{Provider: apiv1.ProviderGitHub, Owner: "third", Name: "repo", Branch: "main"},
	}
	...
	if err == nil {
		t.Fatal("Start succeeded, want conflicting parallel repository selections to fail closed")
	}
}

func createSettledWorkspaceRevisionParallelJournal(t *testing.T, runsDir, runID string, machine *workflow.Machine, revisions map[int]*apiv1.WorkspaceRevision) {
	t.Helper()
	...
	stages := map[int]string{1: "lens-a", 2: "lens-b", 3: "lens-c"}
	for branch, revision := range revisions {
		if err := jr.Append(journal.Event{
			Type: journal.EventBranchStarted, Parallel: "fan", Branch: branch,
			BranchName: names[branch], Stage: stages[branch],
		}); err != nil {
			t.Fatalf("append branch %d started: %v", branch, err)
		}
		if err := jr.Append(journal.Event{
			Type: journal.EventStageFinished, Branch: branch, Stage: stages[branch],
			Attempt: 1, Status: string(apiv1.ResultSuccess),
			WorkspaceRevision: revision.DeepCopy(),
		}); err != nil {
			t.Fatalf("append branch %d stage.finished: %v", branch, err)
		}
		if err := jr.Append(journal.Event{
			Type: journal.EventBranchFinished, Parallel: "fan", Branch: branch,
			BranchName: names[branch], BranchStatus: journal.BranchSucceeded,
		}); err != nil {
			t.Fatalf("append branch %d finished: %v", branch, err)
		}
	}
...
func TestRunnerSettledParallelSelectedRepositoryReachesDownstream(t *testing.T) {
	const runID = "workspace-revision-settled-parallel-repository"
	...
	case env := <-collateEnv:
		if env.RepoRef.Owner != "other" || env.RepoRef.Name != "repo" || env.RepoRef.Branch != "main" {
			t.Fatalf("downstream invocation repo ref = %+v, want settled configured repository", env.RepoRef)
		}
	case <-time.After(runnerTestWaitTimeout):
		t.Fatal("post-join invocation was not observed")
	}
...
func TestRunnerConflictingSettledParallelRepositoriesFailClosed(t *testing.T) {
	const runID = "workspace-revision-settled-parallel-conflict"
	machine := parallelRunnerMachine(t, 2, apiv1.WorkspaceScratch)
	first := runnerWorkspaceRevision("other", "repo", strings.Repeat("a", 40))
	second := runnerWorkspaceRevision("third", "repo", strings.Repeat("b", 40))
	first.BaseRepository, second.BaseRepository = nil, nil
	...
	createSettledWorkspaceRevisionParallelJournal(t, runsDir, runID, machine, map[int]*apiv1.WorkspaceRevision{
		1: first, 2: second,
	})
	...
	if err == nil || (!strings.Contains(err.Error(), "workspace_revision_conflict") &&
		!strings.Contains(err.Error(), "conflicting configured repositories")) {
		t.Fatalf("Resume error = %v, want conflicting settled repository selections", err)
	}
}
...
func TestRunnerParallelBranchPreservesBaseRepositoryWhenNoRevisionSelected(t *testing.T) {
	const runID = "parallel-repository-preserve-base"
	byTask := map[string]stubTaskResult{
