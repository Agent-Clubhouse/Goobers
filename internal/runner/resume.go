...
func (r *Runner) resumeOwned(ctx context.Context, in ResumeInput, jr *journal.Run, registrar SecretRegistrar, dir string) (Result, error) {
	...
	if t, isTask := in.Machine.Task(startState); isTask {
		...
		} else if hasSegmentLast && segmentLastStage == startState {
	...
	result, err := r.walk(ctx, jr, startIn, startState, resume, gateRepassSeed(segment), gateDiffSeed(segment), registrar, seed)
	if err != nil {
		span.Fail(err)
		return result, err
	}
