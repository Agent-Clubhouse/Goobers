...
func (s *fakeNativeSystem) Run(_ context.Context, name string, args []string, stdin string) error {
	...
	s.runStdin = stdin
	s.runs = append(s.runs, fakeNativeRun{
		name:  name,
...
func TestSayEscapesEmbeddedSpeechCommands(t *testing.T) {
	system := &fakeNativeSystem{
		paths:          map[string]string{"say": "/usr/bin/say"},
		output:         "Alex en_US # Most people recognize me\n",
		audioAvailable: true,
	...
	if _, err := sink.Preflight(context.Background()); err != nil {
		t.Fatalf("Preflight: %v", err)
	}

	text := "before [[slnc 1000]] middle [[cmnt suppress me]] after"
	if _, err := sink.Deliver(context.Background(), Request{NotificationID: "say-markup", Text: text}); err != nil {
		t.Fatalf("Deliver: %v", err)
	}
	want := "before " +
		"[[dlim (( ))]][[((dlim [[ ]]))slnc 1000[[dlim (( ))]]]]((dlim [[ ]]))" +
		" middle " +
		"[[dlim (( ))]][[((dlim [[ ]]))cmnt suppress me[[dlim (( ))]]]]((dlim [[ ]]))" +
		" after"
	if system.runStdin != want {
		t.Fatalf("stdin = %q, want %q", system.runStdin, want)
	}
}
...
func TestESpeakSplitsEmbeddedPhonemeMarkup(t *testing.T) {
	system := &fakeNativeSystem{
	...
	text := "before [[h@l'oU]] after"
	if _, err := sink.Deliver(context.Background(), Request{NotificationID: "espeak-markup", Text: text}); err != nil {
		t.Fatalf("Deliver: %v", err)
	}
	if len(system.runs) != 3 {
		t.Fatalf("runs = %#v, want 3 chunks", system.runs)
	}
	var delivered strings.Builder
	for _, run := range system.runs {
		delivered.WriteString(run.stdin)
		if strings.Contains(run.stdin, "[[") || strings.Contains(run.stdin, "]]") {
			t.Fatalf("eSpeak command markup leaked into one invocation: %q", run.stdin)
		}
		if run.name != "/usr/bin/espeak-ng" || strings.Join(run.args, "|") != "-s|180" {
			t.Fatalf("run = %q %#v", run.name, run.args)
		}
	}
	if delivered.String() != text {
		t.Fatalf("delivered text = %q, want %q", delivered.String(), text)
	}
}
...
func TestPreflightRejectsUnusableDefaultVoiceList(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "espeak missing file", platform: "linux", paths: map[string]string{"espeak": "/usr/bin/espeak"}, output: "Pty Language Age/Gender VoiceName File Other Languages\n5 en junk bogus\n"},
	} {
		t.Run(test.name, func(t *testing.T) {
