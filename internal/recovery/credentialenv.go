...
func validateCredentialEnvironment(environment []string) error {
	...
	for _, entry := range environment {
		...
		case "GIT_CONFIG_COUNT", "GIT_CONFIG_KEY_0", "GIT_CONFIG_VALUE_0":
			config[key] = value
		default:
			return fmt.Errorf("recovery credentials contain a non-authentication Git override")
		}
	}
	if len(config) != 0 && (len(config) != 3 || config["GIT_CONFIG_COUNT"] != "1" || config["GIT_CONFIG_KEY_0"] != "credential.helper" || config["GIT_CONFIG_VALUE_0"] != "") {
