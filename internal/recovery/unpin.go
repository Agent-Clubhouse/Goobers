...
func DeleteSnapshotRef(ctx context.Context, repository string, record Record) error {
	...
	err := recoveryGit(ctx, repository, io.Discard, "symbolic-ref", "--quiet", record.Ref)
	if err == nil {
	...
	if err := recoveryGit(ctx, repository, io.Discard, "update-ref", "--no-deref", "-d", record.Ref, record.SnapshotSHA); err != nil {
		...
		verify := "verify " + record.Ref + " " + strings.Repeat("0", len(record.SnapshotSHA)) + "\n"
