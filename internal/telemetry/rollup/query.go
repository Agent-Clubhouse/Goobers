...
func (db *DB) RunRefPage(ctx context.Context, filter RunListFilter, cursorStartedAt time.Time, cursorRunID string, limit int) ([]RunRef, error) {
	query, args, empty := runRefPageQuery(filter, cursorStartedAt, cursorRunID, limit)
	if empty {
		return nil, nil
	}
	return db.runRefRows(ctx, query, args)
}
...
func (db *DB) LatestWorkflowRunRefs(ctx context.Context, gaggle, workflow string) ([]WorkflowRunRef, error) {
	query := `
...
// at tick N" (issue #128).
func (db *DB) SchedulerEvents(ctx context.Context, workflow string) ([]SchedulerEvent, error) {
	query := `
